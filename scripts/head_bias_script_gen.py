...
initial_wd = wd = 0.048831350023898414
defaults = [
    {'corrected': '', 'ep': 90, 'momentum': 0.1, 'lr': 0.011584472366059664, 'sign_lr': 0.1, 'c_sq': 0.8396893026590251, 'wd': None, 'sign_wd': 0.00282842712474619, 'hb_wd': wd, 'nesterov': '', 'cos_power': None, 'power': None},
    {'corrected': None, 'ep': 90, 'momentum': 0.1, 'lr': 0.011584472366059664, 'sign_lr': 0.1, 'c_sq': None, 'wd': wd, 'sign_wd': 0.00282842712474619, 'hb_wd': wd, 'nesterov': '', 'cos_power': None, 'power': None},
]

file_prefixes = ['corrected_head_bias_', 'head_bias_']

for default, file_prefix in zip(defaults, file_prefixes):
    ...
    with open(file_prefix + "training_budgets.sh", "w") as f:
        ...
        done = test_training_budgets(default=default, eps=[300], f=f)

    if not done:
        sys.exit()

pathlib.Path('head_bias_done').touch()
print('Done!')

# [prefix + file for prefix in file_prefixes for file in ["wd.sh", "training_budgets.sh"]] + ['head_bias_done']
# ['corrected_head_bias_wd.sh', 'corrected_head_bias_training_budgets.sh', 'head_bias_wd.sh', 'head_bias_training_budgets.sh', 'head_bias_done']