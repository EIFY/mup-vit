...
class JointCsqLRTuner(AutoTuner):
    ...
    def __init__(self, factor, curr, f):
        ...
        self.factor = factor
        super().__init__(initial_values=[{'c_sq': curr['c_sq'], 'bias_c_sq': curr['bias_c_sq'], 'lr': curr['lr']}], curr=curr, f=f)

    def next_value(self):
        curr = self.values[-1]
        next_val = {'c_sq': curr['c_sq'] * self.factor, 'bias_c_sq': curr['bias_c_sq'] * self.factor, 'lr': curr['lr'] * math.sqrt(self.factor)}
        return next_val, True

    def prev_value(self):
        curr = self.values[0]
        prev_val = {'c_sq': curr['c_sq'] / self.factor, 'bias_c_sq': curr['bias_c_sq'] / self.factor, 'lr': curr['lr'] / math.sqrt(self.factor)}
        return prev_val, True
...
default = {'corrected': '', 'ep': 90, 'momentum': 0.1, 'lr': 0.011584472366059664, 'sign_lr': 0.1, 'c_sq': c_sq, 'wd': None, 'sign_wd': 0.00282842712474619, 'bias_c_sq': c_sq, 'nesterov': '', 'cos_power': None, 'power': 1.1}

file_prefix = 'biased_corrected_'
...
with open(file_prefix + "bias.sh", "w") as f:
    ...
    print(preface, file=f)
    print("# Corrected bias WD tuning:", file=f)
    key = 'bias_c_sq'
    initial_wd = default[key]
    tuner = LRAutoTuner(key, initial_wd, 2 ** 0.5, default, f)
    default, final_acc = tuner.run()
